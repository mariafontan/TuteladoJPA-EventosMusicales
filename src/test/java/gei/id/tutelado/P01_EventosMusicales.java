...
public class P01_EventosMusicales {
    ...
    private static gei.id.tutelado.dao.FestivalDaoJPA festivalDao;
    ...
    public static void setupClass() {
        ...
        festivalDao = new gei.id.tutelado.dao.FestivalDaoJPA();
        festivalDao.setup(cfg.get("EMF"));
    ...
    // Recuperación por clave natural
    @Test
    ...
    @Test
    public void test02b_RecuperaPorCodigoFestival() {
        // Asumimos que no hay festivales en los datos de prueba iniciales, así que
        // creamos uno primero
        gei.id.tutelado.model.Festival festival = new gei.id.tutelado.model.Festival(3001, "Festival de Ortigueira",
                java.time.LocalDateTime.of(2025, 7, 10, 12, 0), "Ortigueira", 4);
        festivalDao.almacena(festival);

        System.out.println("\n\tRecuperando festival por código...");
        gei.id.tutelado.model.Festival recuperado = festivalDao.recuperaPorCodigo(3001);

        assertNotNull("Festival no encontrado", recuperado);
        assertEquals("Código incorrecto", Integer.valueOf(3001), recuperado.getCodigo_evento());
        assertEquals("Nombre incorrecto", "Festival de Ortigueira", recuperado.getNombre());

        System.out.println("\t\tFestival recuperado: " + recuperado);
    }
    ...
    @Test
    public void test04b_AlmacenaFestival() {
        gei.id.tutelado.model.Festival festival;

        System.out.println("\n\tAlmacenando nuevo festival...");
        festival = new gei.id.tutelado.model.Festival(3002, "Resurrection Fest",
                java.time.LocalDateTime.of(2025, 6, 25, 14, 0), "Viveiro", 3);
        festivalDao.almacena(festival);

        System.out.println("\t\tFestival almacenado, recuperándolo...");
        gei.id.tutelado.model.Festival recuperado = festivalDao.recuperaPorCodigo(3002);

        assertNotNull("Festival no recuperado", recuperado);
        assertEquals("Nombre incorrecto", "Resurrection Fest", recuperado.getNombre());
        System.out.println("\t\tFestival recuperado: " + recuperado);
    }
    ...
    @Test
    public void test06b_EliminaFestival() {
        gei.id.tutelado.model.Festival festival;

        System.out.println("\n\tCreando festival temporal...");
        festival = new gei.id.tutelado.model.Festival(3099, "Festival Temporal",
                java.time.LocalDateTime.now(), "Lugar Temporal", 1);
        festivalDao.almacena(festival);

        System.out.println("\t\tEliminando festival...");
        festivalDao.elimina(festival);
    ...
    @Test
    public void test08b_ModificaFestival() {
        gei.id.tutelado.model.Festival festival;

        // Creamos uno para asegurar que existe
        festival = new gei.id.tutelado.model.Festival(3003, "Son do Camiño",
                java.time.LocalDateTime.now(), "Santiago", 3);
        festivalDao.almacena(festival);

        System.out.println("\n\tRecuperando festival para modificar...");
        festival = festivalDao.recuperaPorCodigo(3003);

        System.out.println("\t\tModificando días...");
        festival.setNum_dias(4);
        festivalDao.modifica(festival);

        System.out.println("\t\tRecuperando festival modificado...");
        gei.id.tutelado.model.Festival modificado = festivalDao.recuperaPorCodigo(3003);

        assertEquals("Días no modificados", Integer.valueOf(4), modificado.getNum_dias());
        System.out.println("\t\tFestival modificado: " + modificado);
    }
    ...
    // Propagación automática (CASCADE)
    @Test
    ...
    // Raíz cuadrada de 81
    @Test
    public void test99_RaizCuadrada() {
        System.out.println("\n\tProbando raíz cuadrada de 81...");
        double raiz = Math.sqrt(81);
        assertEquals("La raíz cuadrada de 81 no es 9", 9.0, raiz, 0.0001);
        System.out.println("\t\tCorrecto: sqrt(81) = " + raiz);
    }
}
